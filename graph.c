...
void ProduceOutputFile1(const char *filename, const Graph *g) {
    ...
    // Extract graph name from filename (e.g., "A-SET.TXT" -> "A")
    char graphName[32];
    const char *dash = strstr(filename, "-SET");
    if (dash) {
        int len = dash - filename;
        strncpy(graphName, filename, len);
        graphName[len] = '\0';
    } else {
        strcpy(graphName, "G"); // fallback
    }
